...
module('Integration | Helper | gte', function (hooks) {
  ...
  test('Failure if 1 arg passed in', async function (assert) {
    ...
    this.set('first', '1234');
    await render(hbs`{{gte this.first}}`);
    resetOnerror();
  });
  ...
  test('Failure if more than 2 args passed in', async function (assert) {
    ...
    this.set('third', '1234');
    await render(hbs`{{gte this.first this.second this.third}}`);
    resetOnerror();
  });
  ...
  test('Comparison passes', async function (assert) {
    ...
    await render(hbs`{{gte this.first this.second}}`);

    assert.dom(this.element).hasText('true');

    await render(hbs`{{gte this.first this.first}}`);
